...
def test_shamir():
    ...
    for _ in range(10):
        ...
        q=2**(11*num_words)
        irreducible_poly = get_primitive_poly(11*num_words)
        ...
        shares = share_generation(shared_secret, num_shares, threshold, q, irreducible_poly)
        pick_indices = sample(list(range(1,num_shares+1)), threshold)
        shares_picked = [shares[index-1] for index in pick_indices]
        reconstructed_secret = secret_reconstruction(pick_indices, shares_picked, q, irreducible_poly)
