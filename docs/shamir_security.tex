...
\begin{document}
	...
	\begin{theorem}
		Let $\FF_q$ be a finite field with $q = 2^m$ elements, for some $m \in \ZZ$.
		Our modified Shamir's secret sharing scheme is $(Q,\epsilon)$-secure in  the random oracle model, where $Q$ is the maximum number of queries that any $\ppt$ adversary can make and $\epsilon$ is the success probability of an unbounded adversary against the original Shamir's secret sharing scheme. 
		More precisely, any $\ppt$ adversary $\bdv$ succeeds with probability at most $Q \cdot 2^{-m+d}$, where $d$ is the length of the partial digest of the secret concetanated with an $m - d$ bit randomness.   
	\end{theorem}
