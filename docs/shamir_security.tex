...
\begin{document}
	...
	\begin{theorem}
		...
		More precisely, any $\ppt$ adversary against the SSSd succeeds with probability at most $Q \cdot 2^{d} \cdot \epsilon$.   
	\end{theorem}
	...
	\begin{proof}
		...
		Let $\adv$ be another adversary against the SSS who holds $t-1$ shares, i.e. $\{y_i: i \in I\}$.
		On input $\bdv^{\hash(\cdot)}$ and $\{y_i: i \in I\}$, adversary $\adv$ runs as follows:
		\begin{enumerate}
