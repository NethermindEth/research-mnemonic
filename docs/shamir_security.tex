...
\begin{document}
	...
	\begin{proof}
		...
		Let $\adv$ be another $\ppt$ adversary against the original Shamir's secret sharing scheme who \michals{8.4}{$\adv$ doesn't need to be ppt} holds $t-1$ shares among $n$, i.e.~$y_i \in \FF_q$, $i \in I \subset \{1, \ldots, n\}$ where $I$ is the set of indexes of these $t-1$ shares.
2		On input $\{y_i: i \in I\}$ and  $\bdv^{\hash(\cdot)}$, adversary $\adv$ runs as follows:
		\begin{enumerate}
		 	\item Picks a $d$-bit random partial digest $k' \sample \{0,1\}^{d}$,
		 	\item Runs $\bdv^{\hash(\cdot)}(\{y_i: i \in I\})$, 
		 	\item Guesses a query index $\changedmx{h}{i} \sample \{1, \ldots, Q\}$, where $Q$ is the maximum number of random oracle queries that $\bdv$ can make,
		 	\item Internally simulates the random oracle queries $\hash(R'\concat s')$ \michals{8.4}{What does it mean that $\bdv$ simulates the queries? It takes queries from $\adv$ and responds on them accroding to what you wrote below} and responds $\bdv^{\hash(\cdot)}$'s $j$-th query as follows:
		 	\begin{enumerate}
		 		\item Receives the query $\hash(x)$. \michals{8.4}{It receives $x$ not hash of it.}
		 		\item If $j \neq \changedmx{h}{i}$, then responds with a random $r_\changedmx{j{i}} \sample \{0,1\}^{d}$, \michals{8.4}{The random oracle should be modelled a bit differently. Let it output a string of length equal to the output of the hash function we actually use.}
		 		\item If $j = \changedmx{h}{i}$, then responds with $k'$. \michals{8.4}{See above}
		 	\end{enumerate}
	 		\item Parses $x$ as $R'\concat s'$ and returns $s'$\changedmx{ as the secret of the original Shamir's secret sharing scheme with probability at most $\epsilon$}{}.
		\end{enumerate} 
		...
		\begin{itemize}
		 	...
		 	\item The probability of the correct digest guess, \changedmx{e.i.}{i.e.}~$2^{-d}$ .
		\end{itemize}
