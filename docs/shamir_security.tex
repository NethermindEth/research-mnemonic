...
\begin{document}
	...
	Let $\FF_{q}$ be an extension field where $q = 2^{m}$ for some $m \in \NN$. 
	...
	We define an algorithm $\samplePoly$ which takes the secret $s$, parameters $n, t \in \NN$ and vector $\vec{x} = (x_1, \ldots, x_n)$ as intput. $\samplePoly(s, n, t, \vec{x})$ chooses a random polynomial $p(X)$ from $\sP_{s}^{t - 1}$ and outputs $n$ shares $p(x_1), \ldots, p(x_n)$, cf.~\cref{fig:sample_poly}
	
	\begin{figure}
		\centering
		\begin{pcvstack}[boxed]
			\procedure{$\samplePoly (s, n, t, \vec{x})$}{
