\doxysection{Table of contents}\label{md_research_mnemonic__r_e_a_d_m_e_autotoc_md1}
...
\end{DoxyItemize}\doxysubsection{2. Reconstruction Phase\+:}\label{md_research_mnemonic__r_e_a_d_m_e_autotoc_md6}
...
\doxysubsection{Share Generation}\label{md_research_mnemonic__r_e_a_d_m_e_autotoc_md9}
\doxysubsubsection{INPUT\+: secret $<$em$>$s$<$/em$>$, total number of shares $<$em$>$n$<$/em$>$, threshold $<$em$>$t$<$/em$>$, order of Galois field $<$em$>$q$<$/em$>$, irreducible polynomial of GF($<$em$>$q$<$/em$>$)}\label{md_research_mnemonic__r_e_a_d_m_e_autotoc_md10}
\doxysubsubsection{OUTPUT\+: list of $<$em$>$n$<$/em$>$ shares}\label{md_research_mnemonic__r_e_a_d_m_e_autotoc_md11}
...
\end{DoxyItemize}\doxysubsection{Secret Reconstruction}\label{md_research_mnemonic__r_e_a_d_m_e_autotoc_md12}
\doxysubsubsection{INPUT\+: list of share id $<$em$>$x$<$/em$>$, list of access shares $<$em$>$y$<$/em$>$, order of Galois field $<$em$>$q$<$/em$>$, irreducible polynomial of GF($<$em$>$q$<$/em$>$), digest length $<$em$>$d$<$/em$>$.}\label{md_research_mnemonic__r_e_a_d_m_e_autotoc_md13}
\doxysubsubsection{OUTPUT\+: secret $<$em$>$s\textquotesingle{}$<$/em$>$ or abort.}\label{md_research_mnemonic__r_e_a_d_m_e_autotoc_md14}
...
\item If HMAC-\/\+SHA256({\itshape D\textquotesingle{}}[{\itshape d\+:}] $\vert$$\vert$ {\itshape s\textquotesingle{}})[$\ast$\+:d$\ast$] = {\itshape D\textquotesingle{}}[$\ast$\+:d$\ast$], then return {\itshape s\textquotesingle{}}. Otherwise abort.
\end{DoxyItemize}\doxysection{Basic usage}\label{md_research_mnemonic__r_e_a_d_m_e_autotoc_md15}
...
\end{DoxyItemize}\doxysection{Format of a share}\label{md_research_mnemonic__r_e_a_d_m_e_autotoc_md16}
...
\end{DoxyItemize}
\end{DoxyItemize}\doxysection{Design rationale}\label{md_research_mnemonic__r_e_a_d_m_e_autotoc_md17}
\doxysubsection{Role of the digest}\label{md_research_mnemonic__r_e_a_d_m_e_autotoc_md18}
...
\end{DoxyItemize}\doxysubsection{Number of words and Galois field}\label{md_research_mnemonic__r_e_a_d_m_e_autotoc_md19}
...
\end{DoxyItemize}\doxysubsection{Security}\label{md_research_mnemonic__r_e_a_d_m_e_autotoc_md20}
...
\item Another security issue for our modified scheme is the decreasing entropy because of the digest check that is used in the reconstruction phase. But ensuring that {\itshape m-\/d $>$ 128} can defeat this low entropy weakness.
\item Finally we have another issue for the security. Assume that an attacker has {\itshape k} shares such that {\itshape t $<$ k $<$ n}, and assume that {\itshape e} shares among {\itshape k} are faulty shares. In this case Lagrange interpolation does not work because of the faulty shares. But if {\itshape e $<$ t $<$ k/3} then the {\texttt{ {\itshape Berlekamp-\/\+Welch}}} algorithm works and the adversary can reconstruct the secret without knowing the true shares.
\end{DoxyItemize}\doxysubsection{References}\label{md_research_mnemonic__r_e_a_d_m_e_autotoc_md21}

\begin{DoxyEnumerate}
\item {\texttt{ Shamir, A.\+: How to share a secret. Communications of the Association for Computing Machinery 22(11), 612\+\_\+613 (Nov 1979)}}
\item {\texttt{ BIP-\/39\+: Mnemonic code for generating deterministic keys}}
\item {\texttt{ SLIP-\/0039\+: Shamir\textquotesingle{}s Secret-\/\+Sharing for Mnemonic Codes}} 
\end{DoxyEnumerate}