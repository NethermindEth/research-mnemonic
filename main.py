...
def create_shares(args):
    ...
    * share_i.json (for 1<=i<=n): Contains the following entry:
    ...
    secret_as_list = secret.strip().split(' ')
    nw = len(secret_as_list)
    assert nw in range(12,61), "Secret has " + str(nw) + " words, but it must have between 12 and 60 words"
    assert set(secret_as_list).issubset(set(word_list)), "Not every word in the secret is contained in the dictionary"
    ...
    binary_secret = word_coding.encode_words(word_list, secret.strip())
    ...
    for i in range (len(shares)):
        ...
        reconstructed_shared_secret = word_coding.decode_words(word_list,format(share_with_checksum, "b").zfill(nb * nw))
        file_name = "shares/share_" + str(i+1) + ".json"
        share_data = {
                'share' : reconstructed_shared_secret
                }
        print(reconstructed_shared_secret + '\n-----')
    ...
    for json_filename in list_of_share_filenames:
        ...
        if verbose:
            try:
                ...
                raise RuntimeError('-v flag was added, but not all the shares are verbose.')
        assert len(json_share['share'].split(' ')) == num_words, 'A share has a number of words inconsistent with the given polynomial and dictionary.'
